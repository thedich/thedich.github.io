	...
	public class Path extends EventDispatcher
	{
		...
		public function set points(value:Array):void{
			...
			while(point = iterator.next()){
				point.removeEventListener(BezierEvent.POSITION_CHANGED, onPointPosChange);
				point.removeEventListener(BezierEvent.VECTOR_CHANGED, onPointVectorChange);
			}
			iterator.release();
			...
			var bezier:Bezier;
			while(bezier = iterator.next()){
				bezier.removeEventListener(BezierEvent.SHAPE_CHANGED, invalidateShape);
		...
		public function addPointAt(point:BezierPoint, index:Number):void{
			...
			bezier.dispatchEvents = false;
			_bezierList.splice(index,0,[bezier]);
			
			point.addEventListener(BezierEvent.POSITION_CHANGED, onPointPosChange);
			point.addEventListener(BezierEvent.VECTOR_CHANGED, onPointVectorChange);
		...
		public function removePoint(point:*):void{
			...
			if(index>-1){
				...
				bezPoint.removeEventListener(BezierEvent.POSITION_CHANGED, onPointPosChange);
				bezPoint.removeEventListener(BezierEvent.VECTOR_CHANGED, onPointVectorChange);
		...
		public function drawInto(graphics:Graphics, offset:Point=null):void{
			validateShape();
			var iterator:IIterator = _bezierList.getIterator();
			var bezier:Bezier;
			var first:Boolean = true;
			while(bezier = iterator.next()){
				bezier.drawInto(graphics, first, offset);
				first = false;
			}
			iterator.release()
		}
		protected function linkBeziers():void{
			if(_pointList.length){
				...
				while((bezier = bezIterator.next()) && (point = pointIterator.next())){
					...
					pointClone.dispatchEvents = false;
		...
		protected function _invalidateShape(e:Event=null):void{
			invalidShape = true;
			if(dispatchEvents){
				dispatchEvent(new BezierEvent(BezierEvent.SHAPE_CHANGED));
			}
		}
		protected function onPointPosChange(e:Event=null):void{
			var target:BezierPoint = (e.target as BezierPoint);
			var index:int = _pointList.firstIndexOf(target);
			if(index!=-1){
				linkBeziers();
				var bezier:Bezier = _bezierList.get(index);
				bezier.start.x = target.x;
				bezier.start.y = target.y;
				if(dispatchEvents){
					dispatchEvent(new BezierEvent(BezierEvent.SHAPE_CHANGED));
				}
			}
		}
		protected function onPointVectorChange(e:Event=null):void{
			var target:BezierPoint = (e.target as BezierPoint);
			var index:int = _pointList.firstIndexOf(target);
			if(index!=-1){
				linkBeziers();
				var bezier:Bezier = _bezierList.get(index);
				if(!isNaN(target.forwardAngle))bezier.start.forwardAngle = target.forwardAngle;
				if(!isNaN(target.forwardDistance))bezier.start.forwardDistance = target.forwardDistance;
				if(!isNaN(target.backwardAngle))bezier.start.backwardAngle = target.backwardAngle;
				if(!isNaN(target.backwardDistance))bezier.start.backwardDistance = target.backwardDistance;
				if(dispatchEvents){
					dispatchEvent(new BezierEvent(BezierEvent.SHAPE_CHANGED));
				}
			}
